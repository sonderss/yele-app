  ...
  <view class="appointment-record">
    <view class="top-view" v-if="list.length !== 0">
      <view class="top-v" @click="change1">
        ...
        <image
          :class="animation ? 'arro animation right-arrow-a ':  'arro animation'"
          src="/static/images/downarrow24.png"
        />
      </view>
      <view class="top-v" @click="change">
        <text :class="num === 1 ? 'status end' : 'status' ">消费金额</text>
        <image
          :class="animation1 ? 'arro  animation right-arrow-a'  :  'arro  animation'"
          src="/static/images/downarrow24.png"
        />
      </view>
    </view>
    <view class="main p-tb-20 m-tb-20 m-lr-30 p-lr-20" v-for="(item,index) in list" :key="index">
      ...
      <view class="item">
        台位状态：
        <text
          :class="item.status === 4 || item.status === 5 ? 'status': 'status confirmed'"
...
export default {
  ...
  mounted() {
    const data = { order: "time", sort: "asc", page: 1, limit: 10 };
    this.getData(data);
  },
  methods: {
    getData(data) {
    ...
    change() {
      ...
      if (this.animation1) {
        const data = { order: "price", sort: "desc", page: 1, limit: 10 };
        this.getData(data);
      } else {
        const data = { order: "price", sort: "asc", page: 1, limit: 10 };
        this.getData(data);
      }
    },
    change1() {
      ...
      // desc：倒序，asc：顺序（默认）
      if (this.animation) {
        const data = { order: "time", sort: "desc", page: 1, limit: 10 };
        this.getData(data);
      } else {
        const data = { order: "time", sort: "asc", page: 1, limit: 10 };
        this.getData(data);
