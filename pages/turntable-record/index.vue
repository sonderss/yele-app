...
<view class="turntable-record p-lr-20 p-tb-20">
    <view class="top_view m-bottom-20">
        <min-search placeholder="营销姓名搜索" v-model="key" />
    </view>
    <view v-if="getKeyData.length !== 0">
        <min-cell class="main m-bottom-20" :card="false" v-for="item in getKeyData" :key="item.id" @click="toDeskBill(item.opening_id,item.transfer_desk_id)">
            ...
            <view class="bottm">
                <view class="_left">{{$minCommon.formatDate(new Date(item.create_time*1000),"yyyy/MM/dd hh:mm:ss") }}</view>
        ...
        getKeyData() {
            ...
            if (this.key) {
                this.list.filter(item => {
                    if (item.sales_user_name.toLowerCase().includes(this.key.toLowerCase())) {
                        d.push(item);
                    }
                    // else if (
