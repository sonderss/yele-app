...
<view class="fetch-record p-tb-20 p-lr-30">
    <view class="card p-lr-20 m-bottom-20" v-for="(item,index) in list" :key="index" @click="goDeatail(item.id)">
        <view class="top p-tb-30 min-border-bottom">
            ...
            <view :class="item.fetch_status === 1 ? 'status confirmed':'status end'">{{item.fetch_status === 1 ? '待确认': '已取酒'}}</view>
        </view>
...
export default {
    ...
    methods: {
        async getAllData(page, limit, isLoading){
            return await this.$minApi.getAllWindeList({
                page,
                limit,
                isLoading
            }) 
        ...
        async getData(page, limit, isLoading) {
            return await this.$minApi.getWindeList({
                page,
                limit,
                isLoading
            })
