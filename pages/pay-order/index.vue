        ...
        <view class="discount-wrap m-top-20">
            ...
            <view class="all-yh" v-if="discountType === ALL_YH">
                ...
                <view class="header">
                    ...
                    <input class="input" maxlength="11" type="number" style="text-align:right" v-model="qdyouhui" placeholder="请输入优惠金额" placeholder-style="font-size:28rpx;" />
                </view>
            </view>
            <!--单品打折-->
...
export default {
    ...
    methods: {
        submit() {
            ...
            if (this.kouchu || this.quanyou || this.k) {
                ...
                // }
                const discountData = {
                    signoff_type: this.discountType
                }
                if (this.discountType === 1) {
                    discountData.discount = this.quanyou
    ...
    mounted() {
        this.$minApi.previewOrder({
            order_id: this.$parseURL().data.order_id,
            desk_id: this.$parseURL().data.desk_id,
            open_status: this.$parseURL().data.open_status
        }).then(res => {
            this.list = res
            this.money = this.list.order_info.actual_total
