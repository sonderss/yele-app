...
export default {
  ...
  methods: {
    getData() {
      if (this.product_type === 'product') {
        this.$minApi
          .getOriderProductDetail({ product_id: this.product_id })
          .then(res => {
            this.list = res.info
            this.list.step = 0
        ...
        this.$minApi
          .getOriderServeDetail({ service_id: this.product_id })
          .then(res => {
            this.list = res.info
            this.list.step = 0
          ...
          if (res.orderId) {
            ...
            setTimeout(() => {
              ...
              if (this.$parseURL().is_open_desk) {
                ...
                uni.navigateTo({
                  url:
                    '../placean-order/orderstart?order_id=' +
                    res.orderId +
                    '&desk_id=' +
                    this.$parseURL().desk_id +
                    '&open_status=' +
