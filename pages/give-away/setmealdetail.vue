    ...
    <view class="introduction " v-for="(item,index) in list.combination" :key="index">
        <view>
            ...
            <view class="content p-bottom-30" v-if="item.necessary === 1">
                <min-describe :leftIconValue="item.combination_detail[0].product_img" :leftIcon='true' :leftTxt='item.combination_name' :num="list.combination[0].last_number+''"></min-describe>
            </view>
            ...
            <view class="content p-bottom-30">
                <min-describe class="i" v-for="(item2,index2) in item.combination_detail" :key="index2" :leftIcon='true' :value="item2.step" @changeCount="changeCount($event,index,index2)" :maxStep="item.is_check !== 1 ? (item.goodsCount !== item.last_number ? item.last_number : item.last_number - item.goodsCount) : item.last_number - item.goodsCount" :leftTxt="item2.product_name + `*${item2.num}` " :leftIconValue='item2.comb_type === 1 ? item2.product_img : item2.product_img' :step='type === 3 ? false: true' :Animation="true"></min-describe>
            </view>
        </view>
    </view>
