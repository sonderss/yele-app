...
<view class="storing-liquor-detail p-tb-20 p-lr-30">
    ...
    <view class="card p-lr-20 m-top-20" v-if="list.fetch_product_info.length !== 0">
        ...
        <view class="item">到期时间：{{list.deposit_info.expires_time}}</view> -->
            <view class="item">存酒台号：{{list.deposit_info.desk_name ? list.deposit_info.desk_name :'暂无'}}</view>
            <view class="item" v-if="list.deposit_info.deposit_status !== 1 && list.deposit_info.deposit_status !== 5">确认人员：{{list.deposit_info.confirm_name}}</view>
            <view class="item" v-if="list.deposit_info.deposit_status === 6">取酒人员：{{list.deposit_info.operator_name}}</view>
            <view class="item" v-if="list.deposit_info.deposit_status === 5">操作人员：{{list.deposit_info.operator_name}}</view>
            <view class="item" v-if="list.deposit_info.deposit_status === 5">备注：{{list.deposit_info.operator_remarks}}</view>
            <view class="item" v-if="list.deposit_info.deposit_status !== 1 && list.deposit_info.deposit_status !== 5">生效时间：{{list.deposit_info.confirm_time}}</view>
            <view class="item" v-if="list.deposit_info.deposit_status !== 1  && list.deposit_info.deposit_status !== 5">到期时间：{{list.deposit_info.expires_time}}</view>
        </view>
    </view>
    <!-- <view class="btn-wrap">
