...
<view class="switch-stores m-lr-30 p-top-20">
    ...
    <min-cell :card="false">
        <min-cell-item v-for="(item, index) in storeList" :isWidth="false" isC :key="index" :img="item.head_img ? item.head_img : '/static/images/logo.png'" :title="item.store_name" :label="item.address" :border="index === storeList.length -1 ? false : true" arrow @eventParent="changestore(item)"></min-cell-item>
    </min-cell>
