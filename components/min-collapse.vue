  ...
  <view class="card" >
      ...
      <view v-if="isArray">
        <view class="content"  :class="isShow ? 'animation': 'show animation' "  :style="{'height':isShow? `${list.length%5 == 0 ? list.length / 5 : Math.round(list.length/5) + 1}00`-140+'rpx' :'0','padding-bottom':ism +'rpx'}">
         ...
         <view class="content"  v-if="isKua === 1 || isKua === '1'" :class="isShow ? 'animation': 'show animation' "  :style="{'height':isShow? `${nightArr.length%5 == 0 ? nightArr.length / 5 : Math.round(nightArr.length/5) + 1}00`-140+'rpx' :'0'}">
